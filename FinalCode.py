...
class mainFrame:
    def __init__(self, parent):
        ...
        # Making the label
        self.__main_label = Label(self.main_frame, text="Fleur",
        ...
        self.start_game = Button(self.main_frame, text="Start Game",
                                 font=('Century Gothic', 16),
                                 pady=5, padx=10, command=self.close_main)
        self.start_game.grid(row=3, column=0)

        # Making the how to button
        self.EditButton = Button(self.main_frame, text="Edit Words",
                                 font=('Century Gothic', 16), pady=5, padx=10,
                                 command=self.edit_words)
        self.EditButton.grid(row=3, column=3)

        self.howToPlayButton = Button(self.main_frame, text="How to play",
                                      font=('Century Gothic', 16), pady=5,
                                      padx=10, command=self.HowToPlayFrame)
    ...
    def HowToPlayFrame(self):
        ...
        self.help_heading = Label(self.how_to_frame,
                                  text="The underscores represent"
        ...
        self.how_to_frame.protocol('WM_DELETE_WINDOW',
                                   partial(self.close_HowTo))
    ...
    def edit_words(self):  # arema
        ...
        self.edit_words_label = Label(self.words_edit_screen, text="ID",
                                      width=10, font=("Arial", "14", "bold"),
                                      bg=background)
        self.edit_words_label.grid(row=0, column=1)

        self.edit_words_label2 = Label(self.words_edit_screen,
                                       text="Word", width=10,
                                       font=("Arial", "14", "bold"),
                                       bg=background)
        self.edit_words_label2.grid(row=0, column=3)

        self.edit_words_label9 = Label(self.words_edit_screen, text="default",
                                       width=10, font=('Century Gothic', 16),
                                       bg=background)
        self.edit_words_label9.grid(row=1, column=1)

        self.edit_words_label10 = Label(self.words_edit_screen,
                                        text="default", width=10,
                                        font=('Century Gothic', 16),
                                        bg=background)
        self.edit_words_label10.grid(row=2, column=1)

        self.edit_words_label11 = Label(self.words_edit_screen,
                                        text="default", width=10,
                                        font=('Century Gothic', 16),
                                        bg=background)
        self.edit_words_label11.grid(row=3, column=1)

        self.edit_words_label7 = Label(self.words_edit_screen,
                                       text="Hangman", width=10,
                                       font=('Century Gothic', 16),
                                       bg=background)
        self.edit_words_label7.grid(row=1, column=3)

        self.edit_words_label8 = Label(self.words_edit_screen,
                                       text="Fleur",
                                       width=10,
                                       font=('Century Gothic', 16),
                                       bg=background)
        ...
        self.edit_words_label12 = Label(self.words_edit_screen,
                                        text="Game", width=10,
                                        font=('Century Gothic', 16),
                                        bg=background)
        self.edit_words_label12.grid(row=3, column=3)

        self.edit_words_label3 = Label(self.words_edit_screen, text="",
                                       width=10,
                                       font=("Arial", "14", "bold"),
                                       bg=background)
        ...
        self.edit_words_label4 = Label(self.words_edit_screen, text="\nEdit:"
                                       "To edit an existing word, enter its"
                                       "id\n"
                                       "in the entry box\n"
                                       "and enter the updated value"
                                       "in the\n popup window."
                                       "Press finished\n"
                                       "when done", width=40,
        ...
        self.edit_words_label5 = Label(self.words_edit_screen,
                                       text="\nDelete: Enter the Id"
                                       " into the entry box\n"
                                       "and then click delete", width=40,
                                       font=("Arial", "10"), bg=background)
        self.edit_words_label5.grid(row=2, column=2)

        self.edit_words_label6 = Label(self.words_edit_screen, text="\nAdd"
                                       ": Enter the word into the entry box\n"
                                       "and then click add", width=40,
                                       font=("Arial", "10"), bg=background)
        ...
        with db_conn:
            ...
            for row in rows:
                ...
                Label(self.words_edit_screen, text=_idrowthing,
                      width=10, font=('Century Gothic', 16),
                      bg=background).grid(row=rowcounter, column=1)
                Label(self.words_edit_screen, text=_rowthing,
                      width=10, font=('Century Gothic', 16),
                      bg=background).grid(row=rowcounter, column=3)
                rowcounter += 1
        ...
        def adding_words_button_action():
            ...
            def correct(inp):
                if inp == " ":
                    ...
                    messagebox.showwarning("Error",
                                           "Special chracters are not allowed")
                    return False
            ...
            self.newWordsList = []
            self.userInput = ""
            self.new_words = Entry(self.adding_words_button_frame,
                                   font=('Century Gothic', 16))
            ...
            self.adding_words_button_frame.protocol('WM_DELETE_WINDOW',
                                                    partial
                                                    (self.close_add_words))

            self.new_word_button = Button(self.adding_words_button_frame,
                                          text="Add",
                                          font=('Century Gothic', 16),
                                          pady=5, padx=10,
                                          command=enter_new_word_db)
        ...
        def edit_words_button_action():
            ...
            reg = root.register(correct)
            self.edit_words_button_entry = Entry(self.edit_words_button_frame,
                                                 font=('Century Gothic', 16))
            self.edit_words_button_entry.grid(row=0, column=1)

            self.edit_words_button_entry.config(validate="key",
            ...
            def edit_them():
                ...
                if len(thenewvalue) == 0:
                    ...
                    db_conn.execute("""
                                    UPDATE hangingGame SET name=? WHERE ID=?
                                    """, (thenewvalue, editWordsInput,))
                    db_conn.commit()
                    refresh_words()
                    self.edit_words_button_frame.destroy()
                    self.edit_words_button.config(state=NORMAL)

            self.edit_value_button = Button(self.edit_words_button_frame,
                                            text="Finished",
                                            font=('Century Gothic', 16),
                                            pady=5, padx=10,
                                            command=edit_them)
            self.edit_value_button.grid(row=0, column=2)
            self.edit_words_button_frame.protocol('WM_DELETE_WINDOW',
                                                  partial
                                                  (self.close_edit_words))

        def delete_words_button_action():
            try:
                editWordsInput = self.edit_words_entry.get()
                db_conn.execute("DELETE FROM hangingGame WHERE ID = ?",
                                (editWordsInput,))
                db_conn.commit()
                refresh_words()
        ...
        self.edit_words_entry = Entry(self.words_edit_screen,
                                      font=('Century Gothic', 16),
                                      text="test", )
        self.edit_words_entry.grid(row=900, column=2)

        self.edit_words_button = Button(self.words_edit_screen,
                                        text="Edit words",
                                        font=('Century Gothic', 16),
                                        pady=5, padx=10,
                                        command=edit_words_button_action)
        self.edit_words_button.grid(row=901, column=1)

        self.delete_words_button = Button(self.words_edit_screen,
                                          text="Delete words",
                                          font=('Century Gothic', 16),
                                          pady=5, padx=10,
                                          command=delete_words_button_action)
        self.delete_words_button.grid(row=901, column=2)

        self.add_words_button = Button(self.words_edit_screen,
                                       text="Add words",
                                       font=('Century Gothic', 16),
                                       pady=5, padx=10,
                                       command=adding_words_button_action)
        self.add_words_button.grid(row=901, column=3)

        self.words_edit_screen.protocol('WM_DELETE_WINDOW',
                                        partial(self.close_Settings))
...
def insert_Value(koko):
    ...
    db_conn.execute("INSERT OR REPLACE INTO hangingGame(name) VALUES (?)",
                    (koko,))
    db_conn.commit()
...
class MainGame:
    def __init__(self):
        ...
        global photos
        photos = [PhotoImage(file="hang0.png"),
                  PhotoImage(file="hang1.png"),
                  PhotoImage(file="hang2.png"),
                  PhotoImage(file="hang3.png"),
                  PhotoImage(file="hang4.png"),
                  PhotoImage(file="hang5.png"),
        ...
        lblWord = StringVar()
        self.theWord = Label(self.game_frame, textvariable=lblWord,
                             font=("Consolas 24 bold")).grid(row=0,
                                                             column=3,
                                                             columnspan=6,
                                                             padx=10)
        ...
        for c in ascii_uppercase:
            self.theLetters = Button(self.game_frame,
                                     text=c, command=lambda c=c: self.guess(c),
                                     font=("Helvetica 18"),
                                     width=4).grid(row=1+n//9,
                                                   column=n % 9)
            n += 1
        ...
        if numberOfGuesses < 11:
            ...
            if the_word_withSpaces.count(letter) > 0:
                ...
                if lblWord.get() == the_word_withSpaces:
                    ...
                    self.restart_frame = Toplevel(bg=background,
                                                  pady=10, padx=10)
                    self.play_again_button = Button(self.restart_frame,
                                                    text="Play Again",
                                                    command=self.close_game,
                                                    font=("Consolas 24 bold"))
                    self.play_again_button.grid(row=0, column=3, columnspan=6,
                                                padx=10)
            else:
                numberOfGuesses += 1
                self.imgLabel.config(image=photos[numberOfGuesses])
                if numberOfGuesses == 11:
                    messagebox.showwarning("Oh no!", "You Lose")
                    self.restart_frame = Toplevel(bg=background,
                                                  pady=10, padx=10)
...
# this code is just testing. will connect the
# game and main components in the final version
